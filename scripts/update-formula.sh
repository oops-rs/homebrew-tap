...
Examples:
  ...
  scripts/update-formula.sh face v0.0.1
...
grapha_bridge_install_path_for_tag() {
  local tag="$1"
  local version="${tag#v}"
  ...
  if (( major > 0 || minor > 3 || (minor == 3 && patch >= 1) )); then
    printf '%s\n' "target/release/libGraphaSwiftBridge.dylib"
  else
    printf '%s\n' "grapha-swift/swift-bridge/.build/release/libGraphaSwiftBridge.dylib"
  fi
}
...
render_formula() {
  ...
  case "${formula_name}" in
    grapha)
      local grapha_bridge_path
      grapha_bridge_path="$(grapha_bridge_install_path_for_tag "${formula_tag}")"
      cat <<EOF
...
class ${class_name} < Formula
  ...
  def install
    ...
    libexec.install "${grapha_bridge_path}"
...
class ${class_name} < Formula
  ...
  url "https://github.com/oops-rs/face/archive/refs/tags/${formula_tag}.tar.gz"
  sha256 "${archive_sha}"
  license "MIT"
